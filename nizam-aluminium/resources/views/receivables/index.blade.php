...
@section('title', 'Daftar Piutang & Status')

@section('content')
<div class="mb-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
    <div>
        <h2 class="text-2xl font-extrabold text-gray-800">Kelola Piutang & Status</h2>
        <p class="text-sm text-gray-500 mt-1">Pantau sisa tagihan pelanggan dan input penerimaan uang (DP/Cicilan).</p>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="p-4 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-gray-50">
        <form action="{{ route('reports.receivables') }}" method="GET" class="flex flex-col sm:flex-row items-center gap-2 w-full md:w-auto">
            ...
            @if(request('search'))
                <a href="{{ route('reports.receivables') }}" class="bg-red-50 hover:bg-red-100 text-red-600 px-5 py-2.5 rounded-lg text-sm font-semibold transition border border-red-100 w-full sm:w-auto text-center">Reset</a>
            @endif
        </form>
    </div>
        ...
        <table class="w-full text-sm text-left text-gray-500 min-w-max">
            <thead class="text-[11px] text-gray-400 uppercase bg-white border-b border-gray-100 tracking-wider">
                <tr>
                    ...
                    <th class="px-6 py-4 font-bold text-right text-gray-800">Total Tagihan (Deal)</th>
                    <th class="px-6 py-4 font-bold text-right text-red-600">Sisa Piutang</th>
            ...
            <tbody>
                ...
                @php
                    // Menghitung otomatis piutang untuk ditampilkan di tabel
                    $terbayar = $order->payments()->sum('amount');
                    $sisa = $order->total_price - $terbayar;
                @endphp
                <tr class="bg-white border-b border-gray-50 hover:bg-blue-50/30 transition">
                    ...
                    <td class="px-6 py-4 text-right">
                        @if($sisa > 0)
                            <span class="font-bold text-red-600">Rp {{ number_format($sisa, 0, ',', '.') }}</span>
                        @else
                    ...
                    <td class="px-6 py-4 text-center">
                        @if($order->status == 'ongoing')
                            <span class="text-xs font-bold text-yellow-600">Sedang Dikerjakan</span>
                        @else
                            <span class="text-xs font-bold text-blue-600">Diserahkan (Selesai)</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-center">
                        <a href="{{ route('receivables.edit', $order->id) }}" class="inline-flex items-center gap-1 p-2 bg-green-50 text-green-600 hover:bg-green-100 rounded-lg transition border border-green-100 text-xs font-bold shadow-sm">
                ...
                @empty
                <tr><td colspan="6" class="px-6 py-12 text-center text-gray-500">Belum ada data tagihan. Buat Harga Deal terlebih dahulu di menu Penawaran.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
