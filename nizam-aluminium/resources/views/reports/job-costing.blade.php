...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="p-4 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-gray-50">
        <form action="{{ route('reports.jobCosting') }}" method="GET" class="flex flex-col sm:flex-row items-center gap-2 w-full md:w-auto">
            ...
            <input type="month" name="month" value="{{ request('month') }}" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full sm:w-auto p-2.5">
        ...
        <a href="{{ request()->fullUrlWithQuery(['export' => 'excel']) }}" class="bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-lg text-sm font-semibold transition flex items-center gap-2 shadow-sm w-full md:w-auto justify-center">
    ...
    <div class="overflow-x-auto custom-scrollbar">
        <table class="w-full text-sm text-left text-gray-500 min-w-max">
            ...
            <tbody>
                ...
                @empty
                <tr><td colspan="6" class="px-6 py-12 text-center text-gray-500">Belum ada data laporan laba/rugi untuk kriteria ini.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
