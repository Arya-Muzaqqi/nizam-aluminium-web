<table>
    ...
    <tr>
        <td colspan="9" style="text-align: center; font-weight: bold; font-size: 14px;">BENGKEL NIZAM ALUMINIUM & KACA</td>
    </tr>
    ...
    <tr>
        ...
        <th style="font-weight: bold; border: 1px solid #000; text-align: center; background-color: #f3f4f6;">Harga Kesepakatan (Deal)</th>
        <th style="font-weight: bold; border: 1px solid #000; text-align: center; background-color: #f3f4f6;">Modal Material (Rp)</th>
        <th style="font-weight: bold; border: 1px solid #000; text-align: center; background-color: #f3f4f6;">Modal Upah (Rp)</th>
    ...
    @foreach($orders as $order)
        @php
            ...
            $grand_total_deal += $order->total_price;
            $grand_total_material += $biaya_material;
            $grand_total_upah += $biaya_upah;
            $grand_total_hpp += $order->total_hpp;
            $grand_total_laba += $laba_bersih;
        @endphp
        <tr>
            ...
            <td style="border: 1px solid #000;">{{ $order->customer->name ?? 'Umum' }} - {{ $order->project_name }}</td>
            ...
            <td style="border: 1px solid #000; text-align: right;">{{ number_format($biaya_material, 0, ',', '.') }}</td>
            <td style="border: 1px solid #000; text-align: right;">{{ number_format($biaya_upah, 0, ',', '.') }}</td>
            <td style="border: 1px solid #000; text-align: right; background-color: #fce4ec;">{{ number_format($order->total_hpp, 0, ',', '.') }}</td>
            <td style="border: 1px solid #000; text-align: right; font-weight: bold; background-color: #e8f5e9;">{{ number_format($laba_bersih, 0, ',', '.') }}</td>
        </tr>
    @endforeach

    <tr>
        <td colspan="4" style="text-align: right; font-weight: bold; border: 1px solid #000; background-color: #e5e7eb;">GRAND TOTAL:</td>
        <td style="font-weight: bold; text-align: right; border: 1px solid #000; background-color: #f3f4f6;">{{ number_format($grand_total_deal, 0, ',', '.') }}</td>
        <td style="font-weight: bold; text-align: right; border: 1px solid #000; background-color: #f3f4f6;">{{ number_format($grand_total_material, 0, ',', '.') }}</td>
        <td style="font-weight: bold; text-align: right; border: 1px solid #000; background-color: #f3f4f6;">{{ number_format($grand_total_upah, 0, ',', '.') }}</td>
